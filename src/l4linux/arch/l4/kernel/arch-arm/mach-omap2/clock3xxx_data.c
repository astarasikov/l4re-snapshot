...
//		omap_96m_alwon_fck = omap_96m_alwon_fck_3630;
	...
	{
	        // HACK: remap register
	        if (c->lk.clk->clksel_reg)
	        {
	                unsigned int clk_reg = c->lk.clk->clksel_reg - 0xb2000000;

	                if ( (clk_reg & 0xFFFF0000) == 0x48300000 ) {
	                        // prcm
	                        c->lk.clk->clksel_reg = clk_reg - OMAP3430_PRM_BASE + prm_base;
                        } else if ( (clk_reg & 0xFFFF0000) == 0x48000000 ) {
                                // cm
                                c->lk.clk->clksel_reg = clk_reg - OMAP3430_CM_BASE + cm_base;
                        }
                        else {
                                //printk("ERROR: unknown base clksel_reg!!!\n");

                        }

	                //printk("omap3xxx_clk_init: %s clksel_reg=%08x clksel_reg(remaped)=%08x\n", c->lk.con_id, clk_reg, c->lk.clk->clksel_reg);
	        }

	        if (c->lk.clk->enable_reg)
                {
                        unsigned int en_reg = c->lk.clk->enable_reg - 0xb2000000;

                        if ( (en_reg & 0xFFFF0000) == 0x48300000 ) {
                                // prcm
                                c->lk.clk->enable_reg = en_reg - OMAP3430_PRM_BASE + prm_base;
                        } else if ( (en_reg & 0xFFFF0000) == 0x48000000 ) {
                                // cm
                                c->lk.clk->enable_reg = en_reg - OMAP3430_CM_BASE + cm_base;
                        }
                        else {
                                //printk("ERROR: unknown base enable_reg!!!\n");
                        }

                        //printk("omap3xxx_clk_init: %s enable_reg=%08x enable_reg(remaped)=%08x\n", c->lk.con_id, en_reg, c->lk.clk->enable_reg);
                }

	        if (c->lk.clk->dpll_data)
                {
	                if (c->lk.clk->dpll_data->mult_div1_reg)
	                {
	                        unsigned int mult_div1_reg = c->lk.clk->dpll_data->mult_div1_reg - 0xb2000000;
	                        if ( (mult_div1_reg & 0xFFFF0000) == 0x48000000 ) {
                                        // cm
	                                c->lk.clk->dpll_data->mult_div1_reg = mult_div1_reg - OMAP3430_CM_BASE + cm_base;
                                }
                                else {
                                        //printk("ERROR: unknown base mult_div1_reg!!!\n");
                                }

                                //printk("omap3xxx_clk_init: %s mult_div1_reg=%08x mult_div1_reg(remaped)=%08x\n", c->lk.con_id, mult_div1_reg, c->lk.clk->dpll_data->mult_div1_reg);
	                }
                        if (c->lk.clk->dpll_data->control_reg)
                        {
                                unsigned int control_reg = c->lk.clk->dpll_data->control_reg - 0xb2000000;
                                if ( (control_reg & 0xFFFF0000) == 0x48000000 ) {
                                        // cm
                                        c->lk.clk->dpll_data->control_reg = control_reg - OMAP3430_CM_BASE + cm_base;
                                }
                                else {
                                        //printk("ERROR: unknown base control_reg!!!\n");
                                }

                                //printk("omap3xxx_clk_init: %s control_reg=%08x control_reg(remaped)=%08x\n", c->lk.con_id, control_reg, c->lk.clk->dpll_data->control_reg);
                        }
                        if (c->lk.clk->dpll_data->autoidle_reg)
                        {
                                unsigned int autoidle_reg = c->lk.clk->dpll_data->autoidle_reg - 0xb2000000;
                                if ( (autoidle_reg & 0xFFFF0000) == 0x48000000 ) {
                                        // cm
                                        c->lk.clk->dpll_data->autoidle_reg = autoidle_reg - OMAP3430_CM_BASE + cm_base;
                                }
                                else {
                                        //printk("ERROR: unknown base autoidle_reg!!!\n");
                                }

                                //printk("omap3xxx_clk_init: %s autoidle_reg=%08x autoidle_reg(remaped)=%08x\n", c->lk.con_id, autoidle_reg, c->lk.clk->dpll_data->autoidle_reg);
                        }
                        if (c->lk.clk->dpll_data->idlest_reg)
                        {
                                unsigned int idlest_reg = c->lk.clk->dpll_data->idlest_reg - 0xb2000000;
                                if ( (idlest_reg & 0xFFFF0000) == 0x48000000 ) {
                                        // cm
                                        c->lk.clk->dpll_data->idlest_reg = idlest_reg - OMAP3430_CM_BASE + cm_base;
                                }
                                else {
                                        //printk("ERROR: unknown base idlest_reg!!!\n");
                                }

                                //printk("omap3xxx_clk_init: %s idlest_reg=%08x idlest_reg(remaped)=%08x\n", c->lk.con_id, idlest_reg, c->lk.clk->dpll_data->idlest_reg);
                        }
                }
	        //
